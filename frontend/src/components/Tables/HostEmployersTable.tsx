...
import { getAllHostEmployers, GetJobPosts } from "@/lib/data/actions";
import { useRouter } from "next/navigation";
import User from "@/types/user";

const HostEmployersTable = ({ hostEmployers }: { hostEmployers: User[] }) => {
  const router = useRouter();
  // const hostEmployers = await getAllHostEmployers();
  return (
    <div
      className="rounded-[10px] bg-white p-4 shadow-1
    ...
    >
      <div className="max-w-full overflow-x-auto p-4">
        ...
        <table className="w-full table-auto">
          ...
          <tbody>
            {hostEmployers.map((hostEmployer: User, index: number) => (
              <tr key={index}>
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 xl:pl-7.5 ${index === hostEmployers.length - 1 ? "border-b-0" : "border-b"}`}
                >
                ...
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 ${index === hostEmployers.length - 1 ? "border-b-0" : "border-b"}`}
                >
                ...
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 xl:pr-7.5 ${index === hostEmployers.length - 1 ? "border-b-0" : "border-b"}`}
                >
                  <div className="flex items-center justify-end space-x-3.5">
                    <button
                      ...
                      onClick={() =>
                        router.push(
                          `/jobposts/hostemployers/${hostEmployer.id}`,
                        )
                      }
                    >
                      <svg
...
export default HostEmployersTable;
