  ...
  return (
    ...
    >
      <div className="max-w-full overflow-x-auto p-4">
        <MainJobTable jobPostsData={jobPosts} />
      </div>
    </div>
  );
};

const MainJobTable = ({ jobPostsData }: { jobPostsData: JobPost[] }) => {
  "use client";
  const [jobPosts, setJobPosts] = useState<JobPost[]>(jobPostsData);

  const handleSubmitOnJobPostModal = async () => {
    // Call the function to create a new job post
    setJobPosts(await GetJobPosts());
  };
  return (
    <>
              ...
              <tr key={index}>
                ...
                >
                  <div className="flex items-center justify-end space-x-3.5">
                    <Link href={`/jobposts/jobposts/${jobPost.id}`}>
