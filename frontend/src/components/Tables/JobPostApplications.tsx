...
import { GetApplications, GetJobPost } from "@/app/actions";
...
const JobPostApplicationTable = ({jobPostId} : JobPostApplicationTableProps) => {
  ...
  useEffect(() => {
    ...
    const fetchApplicationForPost = async () => {
    ...
    fetchJobPost();
    fetchApplicationForPost()
  }, [])

  const handleSubmitOnJobPostModalUpdate = async () =>{
    // Call the function to update a job post
    const data = await GetJobPost(jobPostId);
    if (!data.error) {
      setJobPost(data);
    }
  ...
  return (<>
    <div className="flex flex-col lg:flex-row gap-4">
      ...
      <div className="lg:w-1/4 w-full rounded-[10px] border border-stroke bg-white p-4 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card sm:p-7.5">
        <div className="max-w-full overflow-x-auto p-4">
         
              {/* Search Box */}
              <JobPostDetails jobPost={jobPost || {}} />
              <JobPostsModalFormUpdate jobPost={jobPost || {}} handleSubmitOnJobPostModalUpdate={handleSubmitOnJobPostModalUpdate}/>
        ...
        <div className="max-w-full overflow-x-auto p-4">
          ...
          <tbody>
            {applications.map((application, index) => (
              <tr key={index}>
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 xl:pl-7.5 ${index === applications.length - 1 ? "border-b-0" : "border-b"}`}
                >
                ...
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 ${index === applications.length - 1 ? "border-b-0" : "border-b"}`}
                >
                ...
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 xl:pr-7.5 ${index === applications.length - 1 ? "border-b-0" : "border-b"}`}
                >
                  <div className="flex items-center justify-end space-x-3.5">
                    <button className="hover:text-primary"
                      onClick={() => router.push(`/jobposts/application/${application.id}`)}
                    >
                      <svg
