...
export default function SignupWithSecret({ secretKey }: { secretKey: string }) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    formData.append("secreteKey", secretKey);
    formData.append("password", password);

    try {
      const response = await SignUpUser(state, formData);
      if (response?.message) {
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="mb-4.5">
        <button
          ...
          gap-2 rounded-lg bg-primary p-4 font-medium text-white transition hover:bg-opacity-90"
        >
